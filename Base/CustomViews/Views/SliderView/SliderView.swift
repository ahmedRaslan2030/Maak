...
class SliderView: UIView {
    ...
    @IBInspectable
    var selectedPageColor: UIColor =  UIColor(resource: .secondary) {
        didSet {
            pageControl.currentPageIndicatorTintColor = selectedPageColor
        }
    }
    @IBInspectable
    var unselectedPageColor: UIColor =  UIColor(resource: .mainLightFont) {
        didSet {
...
extension SliderView: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        isReady == true ? cell.imageView.setWith(url: items[indexPath.item]) : ()
        if self.isCard {
            cell.cornerRadius = 8
        } else {
            cell.cornerRadius = 0
